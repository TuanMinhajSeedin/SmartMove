...
def convert_json_to_schedule_format(json_data: Dict, optional_properties: Dict = None) -> List[Dict]:
    ...
    if "extracted_data" in json_data:
        ...
        for table in tables:
            ...
            # Check for special case: "Colombo Arrival" and "Colombo Departure" as one location
            colombo_arrival_idx = None
            colombo_departure_idx = None
            colombo_base_name = None
            
            for idx, header in enumerate(headers):
                header_lower = header.lower().strip()
                if "colombo" in header_lower and "arrival" in header_lower:
                    colombo_arrival_idx = idx
                    # Extract base location name (e.g., "Colombo" from "Colombo Arrival")
                    colombo_base_name = header.split()[0] if header.split() else "Colombo"
                elif "colombo" in header_lower and "departure" in header_lower:
                    colombo_departure_idx = idx
                    if not colombo_base_name:
                        colombo_base_name = header.split()[0] if header.split() else "Colombo"
            
            # Extract location names (skip non-location headers like "Trip Number")
            ...
            for idx, header in enumerate(headers):
                ...
                if any(keyword in header_lower for keyword in location_keywords):
                    continue
                # Skip if it's Colombo Arrival or Departure (we'll handle separately)
                if idx == colombo_arrival_idx or idx == colombo_departure_idx:
                    continue
                # Add as location
                locations.append(header.strip())
                location_indices.append(idx)
            
            # Handle special case: Colombo with Arrival/Departure
            if colombo_arrival_idx is not None and colombo_departure_idx is not None:
                # Create route from Colombo to first destination
                if locations:
                    to_loc = locations[0]
                    to_idx = location_indices[0]
                    from_loc = colombo_base_name
                    
                    schedules = []
                    
                    # Extract schedules from rows
                    for row in rows:
                        if len(row) > max(colombo_departure_idx, to_idx):
                            # Get Colombo terminal times
                            colombo_arrival = row[colombo_arrival_idx] if colombo_arrival_idx < len(row) else None
                            colombo_departure = row[colombo_departure_idx] if colombo_departure_idx < len(row) else None
                            to_time = row[to_idx] if to_idx < len(row) else None
                            
                            # Parse times
                            arrival_to_terminal = parse_time(colombo_arrival)
                            departure_from_terminal = parse_time(colombo_departure)
