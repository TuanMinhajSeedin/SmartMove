        ...
        for table in tables:
            ...
            if location_with_terminals:
                for base_name, terminal_info in location_with_terminals.items():
                    ...
                    # Check position of terminal location in the table
                    # Case 1: Terminal location at the beginning (e.g., "Galle Arrival", "Galle Departure", "Kadawatha")
                    # Case 2: Terminal location in the middle (e.g., "Negombo", "Kadawatha Arrival", "Kadawatha Departure", "Galle")
                    
                    terminal_min_idx = min(arrival_idx, departure_idx)
                    terminal_max_idx = max(arrival_idx, departure_idx)
                    
                    # Check if there are regular locations before and after the terminal location
                    locations_before = [(loc, idx) for loc, idx in zip(locations, location_indices) if idx < terminal_min_idx]
                    locations_after = [(loc, idx) for loc, idx in zip(locations, location_indices) if idx > terminal_max_idx]
                    
                    # Case 1: Terminal location at the beginning - create route to first destination
                    if not locations_before and locations_after:
                        to_loc, to_idx = locations_after[0]
                        from_loc = base_name
                        ...
                        for row in rows:
                            if len(row) > max(departure_idx, to_idx):
                                ...
                                if departure_from_terminal and arrival:
                                    ...
                                    # Extract additional properties from row
                    ...
                    # Case 2: Terminal location in the middle - create routes from before and to after
                    elif locations_before and locations_after:
                        from_loc, from_idx = locations_before[0]
                        to_loc, to_idx = locations_after[0]
                        terminal_loc = base_name
                        
                        # Route 1: from_loc → terminal_loc
                        schedules_route1 = []
                        for row in rows:
                            if len(row) > max(from_idx, arrival_idx):
                                from_time = row[from_idx] if from_idx < len(row) else None
                                arrival_time = row[arrival_idx] if arrival_idx < len(row) else None
                                
                                departure = parse_time(from_time)
                                arrival_to_term = parse_time(arrival_time)
                                
                                if departure and arrival_to_term:
                                    schedule_item = {
                                        "departure": departure,
                                        "arrival": arrival_to_term,
                                        "arrival_to_terminal": arrival_to_term
                                    }
                                    ...
                                    schedule_item.update(optional_properties)
                                    schedules_route1.append(schedule_item)
                        
                        if schedules_route1:
                            converted_data.append({
                                "from": from_loc,
                                "to": terminal_loc,
                                "schedules": schedules_route1
                            })
                        
                        # Route 2: terminal_loc → to_loc
                        schedules_route2 = []
                        for row in rows:
                            if len(row) > max(departure_idx, to_idx):
                                departure_time = row[departure_idx] if departure_idx < len(row) else None
                                ...
                                departure_from_term = parse_time(departure_time)
                                arrival = parse_time(to_time)
                                
                                if departure_from_term and arrival:
                                    schedule_item = {
                                        "departure": departure_from_term,
                                        "arrival": arrival,
                                        "departure_from_terminal": departure_from_term
                                    }
                                    ...
                                    schedule_item.update(optional_properties)
                                    schedules_route2.append(schedule_item)
                        
                        if schedules_route2:
                            converted_data.append({
                                "from": terminal_loc,
                                "to": to_loc,
                                "schedules": schedules_route2
                            })
